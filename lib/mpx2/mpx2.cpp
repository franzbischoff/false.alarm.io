 ...
 *
 *
 * Since not everyone is aware of this, I'll point out the source of divergence
 * in cases where intermediate arithmetic produces nan values (typically via
 * overflow followed by multiplication)
 *
 * The most common assembly implementation of max (which is not compliant with
 * IEEE2018 rules)
 *
 * is
 *
 ...
 *
 * IEEE2018 prefers to propagate nan. Matlab prefers to propagate a finite
 * value, which is error prone.
 *
 *
 * Second note...
 *
 * I have made considerable efforts to improve the stability of lookahead
 * methods wherever possible. I swapped out the method used for difference
 * equations some time ago. This one unfortunately uses 4 arrays rather than 2,
 * but it held up better in stress tests. I provided some preprocessing to trim
 * missing leading and trailign sections and slightly perturb sequences of
 * constants.
 *
 * The last point will be to explicitly detect that divergence has occurred.
 * This is done by explicitly computing the end values for each diagonal
 * excluding the last.
 *
 * This can issue a warning or return the measurements as a "witness" of the
 * calculations.
 *
 * Last note on implementation...
 *
 * This is specialized for AVX2. As it is, this should run quite well on any
 * target that supports AVX2. If I have to include index calculations, I either
 * have to reduce throughput by halving the unroll factor or accept the use of
 * masked writes, which pessimize run times on AMD hardware.
 *
...
// reference version of the main routine, written to match against earlier
// matlab code for the main routine
//

void mpx_ref(double *mpr, double *mpc, double *cv, double *dr_bwd, double *dc_bwd, double *dr_fwd, double *dc_fwd,
             double *invnr, double *invnc, int dcount) {
...
static inline void pearson_inner(double *__restrict cv, double *__restrict mpr, double *__restrict mpc,
                                 const double *__restrict dr_bwd, const double *__restrict dc_bwd,
                                 const double *__restrict dr_fwd, const double *__restrict dc_fwd,
                                 const double *__restrict invnr, const double *__restrict invnc, const int iters) {
  ...
  /*
   ...
   compute initial co-moments. These give the final co-moment values, which are
   also explicitly computable. One way to test whether divergence occurred
   during calculations is to explicitly compute final values and test against
   these.
...
static inline void pearson_edge(double *__restrict cov, double *__restrict mpr, double *__restrict mpc,
                                double *__restrict dr_bwd, double *__restrict dc_bwd, double *__restrict dr_fwd,
                                double *__restrict dc_fwd, double *__restrict invnr, double *__restrict invnc,
                                int rbegin, int dcount) {
 ...
 *
 * mpr is a matrix profile buffer, corresponding to indices 0 ... subsequence
 * count - min separation mpc is a matrix profile buffer, corresponding to
 * indices min separation .... subsequence count
 *
 * They should be allocated separately. Aliasing writes can produce weird bugs,
 * and it's undefined behavior when using restrict.
 *
 * The other difference is that mpr always refers to the subsequence with the
 * lowest index in any comparison. In the initial input of cv, it is uniformly
 * 0, and cv naturally has subsequence count - min separation elements as a
 * result, passed here as "dcount", standing for "diagonal count".
 *
 *
 * It's assumed that any missing data regions have been trimmed from the
 * beginning and end via preprocessing (see my matlab code).
 *
 *
 ...
 *
 *
 * They can be rederived pretty easily starting from the covariance update
...
void compute_self_mp(double *__restrict cv, double *__restrict mpr, double *__restrict mpc, double *__restrict dr_bwd,
                     double *__restrict dc_bwd, double *__restrict dr_fwd, double *__restrict dc_fwd,
                     double *__restrict invnr, double *__restrict invnc, int dcount) {

  constexpr int stride = 32;
  int stridect = dcount / stride;
  for (int s = 0; s < stridect; ++s) {
    // Compute the number of steps over which we can advance
    // cv[i]...cv[i+stride-1], endpoints included
    int d = s * stride;
    int full = dcount - d - (stride - 1);
    pearson_inner(cv + d, mpr, mpc + d, dr_bwd, dc_bwd + d, dr_fwd, dc_fwd + d, invnr, invnc + d, full);
    pearson_edge(cv + d, mpr, mpc + d, dr_bwd, dc_bwd + d, dr_fwd, dc_fwd + d, invnr, invnc + d, full,
                 dcount - d - full);
  }
